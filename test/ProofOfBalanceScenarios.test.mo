    ...
    await test(
      ...
      func() : async () {
        ...
        let stableData : ProposalEngine.StableData<TreasuryProposal> = {
          proposals = BTree.init<Nat, ProposalEngine.ProposalData<TreasuryProposal>>(null);
        ...
        let onValidate = func(content : TreasuryProposal) : async* Result.Result<(), [Text]> {
          ...
          if (content.recipient == "") {
            errors := Array.concat(errors, ["Recipient address cannot be empty"]);
          };
        ...
        let #ok = whaleVote else Runtime.trap("Whale vote failed");
        ...
        let #ok = dolphinVote else Runtime.trap("Dolphin vote failed");
        ...
        let ?currentProposal = engine.getProposal(proposalId) else Runtime.trap("Proposal not found");
        switch (currentProposal.status) {
          case (#open) Debug.print("✓ Proposal remains open (13.5% < 60% threshold)");
          case (_) Runtime.trap("Expected proposal to remain open");
        };
        ...
        let #ok = institutionVote else Runtime.trap("Institution vote failed");

        // Total YES votes: 1M + 200K + 50K + 75K + 5M = ~6.325M (63.25%)
        // This exceeds 60% threshold but should NOT auto-execute in real-time mode
        let ?finalProposal = engine.getProposal(proposalId) else Runtime.trap("Proposal not found");
        switch (finalProposal.status) {
          ...
          case (_) Runtime.trap("Expected real-time proposal to remain open: " # debug_show (finalProposal.status));
        };
    ...
    await test(
      ...
      func() : async () {
        ...
        let nftStableData : ProposalEngine.StableData<NFTProposal> = {
          proposals = BTree.init<Nat, ProposalEngine.ProposalData<NFTProposal>>(null);
          proposalDuration = null;
        ...
        let #ok(proposalId) = createResult else Runtime.trap("Failed to create NFT proposal");
        ...
        let ?_nftProposal = engine.getProposal(proposalId) else Runtime.trap("NFT proposal not found");
    ...
    await test(
      ...
      func() : async () {
        ...
        let crossChainStableData : ProposalEngine.StableData<CrossChainProposal> = {
          proposals = BTree.init<Nat, ProposalEngine.ProposalData<CrossChainProposal>>(null);
          proposalDuration = null;
        ...
        let #ok(proposalId) = createResult else Runtime.trap("Failed to create cross-chain proposal");
        ...
        let ?_crossProposal = engine.getProposal(proposalId) else Runtime.trap("Cross-chain proposal not found");
